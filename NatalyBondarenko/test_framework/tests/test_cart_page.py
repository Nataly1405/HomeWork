...
@pytest.mark.regression
@pytest.mark.cart
def test_move_to_item_page(open_cart_page_with_item, init_item_page):
    """
    A method to verify that we are able to move to item page from the cart page
    """
    item_page = init_item_page
    open_cart_page_with_item.go_to_item_page()
    assert item_page.is_first_item_located() is True, "User didn't succeed go to item page from cart page"
...
def test_go_to_checkout(open_cart_page_with_item):
    ...
    assert cart_page.is_first_name_field_displayed() is True, "The `checkout` button doesn't work"
...
def test_logout_from_cart_page(open_cart_page, open_login_page):
    ...
    """
    open_cart_page.click_menu_button().click_logout_button()
    login_page = open_login_page
    assert login_page.is_login_button_displayed() is True, "Logout doesn't work from cart page"
...
def test_error_on_go_to_checkout_with_empty_cart(open_cart_page):
    ...
    assert cart_page.is_first_name_field_displayed() is True, "User can not go to checkout with empty cart"
